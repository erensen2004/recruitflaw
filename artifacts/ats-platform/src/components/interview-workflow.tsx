...
export function InterviewWorkflowPanel({
  ...
  const openRequestDialog = (mode: ProposalDialogMode) => {
    ...
    setDialogDescription(
      mode === "request"
        ...
        : `Suggest another time for ${candidateName}.`,
    );
  ...
  const declineProposal = async (proposalId: number) => {
    try {
      ...
      await refresh();
      toast({ title: "Availability sent to admin" });
    } catch (error) {
      toast({
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-500">
            {candidateName} {bundleProcess ? "has an active thread." : "has no thread yet."}
          </p>
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="border-b border-slate-200 bg-slate-50/70 px-4 py-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-slate-500">Client requests stay controlled here before vendor coordination.</p>
              </div>
                    ...
                    {request.candidates.map((candidate) => (
                      <div key={candidate.id} className="flex items-center justify-between gap-2 rounded-xl border border-slate-100 px-3 py-2">
                        <div className="min-w-0">
                          ...
                          <p className="truncate text-[11px] text-slate-500">{candidate.vendorCompanyName ?? "Vendor"} • {candidate.status.replace(/_/g, " ")}</p>
                        </div>
                        {roleBase === "/admin" && ["pending_admin", "vendor_replied"].includes(candidate.status) ? (
                          <Button type="button" size="sm" variant="outline" className="h-8 rounded-lg px-2 text-[11px]" onClick={() => setDispatchTarget({ request, candidate })}>
                            {candidate.status === "vendor_replied" ? "Reply" : "Dispatch"}
                          </Button>
      ...
      <InterviewProposalDialog
        ...
        title={dispatchTarget ? `Dispatch ${dispatchTarget.candidate.candidateName}` : "Dispatch interview"}
        description="Convert the client request into a structured proposal for the vendor."
